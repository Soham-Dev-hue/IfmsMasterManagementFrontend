...
export class SaoLevelComponent implements OnInit {
 ...
 confirmToggleStatus(saoLevel: any) {
       Swal.fire({
         title: `Are you sure?`,
         text: `You are about to mark this saoLevel as ${saoLevel.isactive ? 'Inactive' : 'Active'}.`,
         icon: 'warning',
         showCancelButton: true,
         confirmButtonColor: saoLevel.isactive ? '#d33' : '#28a745',
         cancelButtonColor: '#6c757d',
         confirmButtonText: saoLevel.isactive ? 'Yes, deactivate it!' : 'Yes, activate it!',
       }).then((result) => {
         if (result.isConfirmed) {
           // Call the service to update status in the backend
           this.commonService.UpdateSaoLevelStatus(saoLevel.id,saoLevel).subscribe(
             (response) => {
               // Update the local object with the response from backend
               // or toggle the status if backend doesn't return updated object
               saoLevel.isactive = !saoLevel.isactive;
               
               // Show success message
               Swal.fire({
                 title: 'Updated!',
                 text: `The saoLevel has been marked as ${saoLevel.isactive ? 'Active' : 'Inactive'}.`,
                 icon: 'success',
                 timer: 1500
               });
             },
             (error) => {
               ...
               Swal.fire({
                 title: 'Error!',
                 text: 'Failed to update saoLevel status. Please try again.',
                 icon: 'error'
               });
               console.error('Error updating saoLevel status:', error);
             }
           );
         }
       });
